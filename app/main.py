...
def create_app() -> FastAPI:
    ...
    # Legacy compatibility routers preserved from the local fork.
    try:
        from app.api.v1.admin_api import router as legacy_admin_router
        app.include_router(legacy_admin_router, prefix="/v1/admin")
    except Exception as exc:
        logger.warning("legacy admin api unavailable: error={}", exc)

    try:
        from app.api.v1.public_api import router as legacy_public_router
        app.include_router(legacy_public_router, prefix="/v1/public")
    except Exception as exc:
        logger.warning("legacy public api unavailable: error={}", exc)

    try:
        from app.api.v1.video_api import router as legacy_video_router
        app.include_router(legacy_video_router, prefix="/v1")
    except Exception as exc:
        logger.warning("legacy video api unavailable: error={}", exc)

    try:
        from app.api.pages import router as legacy_pages_router
        app.include_router(legacy_pages_router)
    except Exception as exc:
        logger.warning("legacy pages unavailable: error={}", exc)

    # Static assets — mount legacy local-fork directories first, then upstream statics.
    _legacy_statics_dir = Path(__file__).resolve().parent / "static"
    if _legacy_statics_dir.is_dir():
        for subdir in ("admin", "common", "public", "i18n", "vendor", "cf-worker"):
            legacy_dir = _legacy_statics_dir / subdir
            if legacy_dir.is_dir():
                app.mount(
                    f"/static/{subdir}",
                    StaticFiles(directory=legacy_dir),
                    name=f"legacy-static-{subdir}",
                )

    # Static assets — new statics root.
